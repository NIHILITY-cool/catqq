...
如果命令里的名字或 QQ 号能在 `CATQQ_CONTACTS` 中找到，插件会解析出目标、意图、内容和到点时间。不写时间就是现在发；写了时间会保存到 `data/cat_guard_state.json` 的 `pending_tasks` 中，由每分钟调度器到点发送。

支持的时间表达包括：

- 现在、马上、立刻
- 10分钟后、半小时后、2小时后
- 下午三点、今晚九点、明早八点、明天下午三点

发送时仍然通过 `send_by_session()` 给目标联系人发私聊，并给发起人回复确认。这样"提醒鲍鲍"、"去问问鲍鲍"不会进入普通 AI 对话，也不会依赖当前会话历史去猜鲍鲍有没有来过。
