...
小猫任务和主动联系对象共用 `CATQQ_CONTACTS`，但语义不同：

- 主动联系对象：插件按规则自己决定什么时候找一个固定对象
- 小猫任务：白名单用户明确交代小猫现在或到点后联系某个对象

小猫任务在进入 LLM 前解析。推荐入口是固定前缀 + 自然语言内容：
...
`小猫任务` 前缀用于把消息明确交给任务系统，不再交给 LLM 闲聊。冒号后仍然可以用自然话，插件会解析目标、动作、内容和到点时间。如果名字或 QQ 号能在 `CATQQ_CONTACTS` 中找到，不写时间就是现在发；写了时间会保存到 `data/cat_guard_state.json` 的 `pending_tasks` 中，由每分钟调度器到点发送。

为了兼容日常使用，少数明确直说的句式也会拦截，例如"现在去给鲍鲍考试加油"、"去问问鲍鲍吃药没"。缺目标、缺动作、指代不清的普通聊天不会被猜测执行。
...
发送时仍然通过 `send_by_session()` 给目标联系人发私聊，并给发起人回复确认。因为这类消息会在插件层 `stop_event()`，不会自然进入 LLM 流程，所以插件会额外用 `conversation_manager.add_message_pair()` 写入会话记忆：

- 发起人会话：记录"谁让小猫做了什么"和小猫的确认回复
- 目标人会话：记录"谁让小猫带话/提醒/询问"和小猫实际发出的内容

这样"小猫任务：去问问鲍鲍"不会进入普通 AI 对话，也不会依赖当前会话历史去猜鲍鲍有没有来过；后续鲍鲍问"谁让你带话"，小猫也能从她自己的会话历史里看到任务记录。

任务管理也走同一个入口：

- `小猫任务：列表` 只列出待发送任务
- `小猫任务：取消 #任务ID` 把待发送任务标记为 `cancelled`

非命令式的普通聊天不会被拦截，例如"鲍鲍今天来了吗"仍然交给 AI 正常回复。联系人自己说出明确时间和"记得来找我"时，会被保存成到点联系她的任务，避免 AI 只是口头答应但实际没有调度。
