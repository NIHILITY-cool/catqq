...
- 小猫任务工具：自然对话里让小猫带话、询问、提醒、安排未来任务、查看/取消任务；LLM 先按人设判断，再调用插件提供的结构化工具
...
维护 QQ 表情：先更新 `FACE.md`。人设里的表情说明、程序里的表情映射、测试里的表情样例都要跟随这个模板，不要各写一套。

让小猫执行任务：自然说 `你现在去给鲍鲍说考试加油`、`下午三点问问鲍鲍考完了吗`、`小猫看看你还记着什么任务`。不要再使用旧的 `小猫任务：...` 固定入口；是否执行、怎么措辞由玖玖按人设判断，然后由结构化工具执行。目标收到的是玖玖自然写出的正文，来源记录会进记忆，不会固定拼成"某某让小猫..."。
