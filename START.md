...
联系人配置是身份识别、白名单、主动联系、小猫任务的共同基础。编辑 `.env`：
...
小猫任务是你明确交代小猫去联系某个联系人，适合"现在去问"、"下午三点再问"这种场景。推荐写法是固定入口 + 自然话：
...
小猫任务会直接私聊目标联系人，并给发起人回复确认。任务执行或排程后，会把记录写入相关会话记忆：发起人会记得自己交代过任务，目标联系人也会记得小猫刚刚带了谁的话。带"小猫任务："前缀的消息一定走任务系统，不交给 AI 猜，避免出现"鲍鲍没找我"这类误判。

时间可以不写。不写就是现在发；写了就存成任务，到点后自动发。支持"现在/马上/立刻"、"10分钟后/半小时后/2小时后"、"下午三点/今晚九点/明早八点/明天下午三点/16:00"这类表达。时间可以放在动作前，也可以放在联系人后，例如"半小时后提醒鲍鲍喝水"和"提醒鲍鲍半小时后喝水"都可以。

为了日常使用顺手，少数很明确的直接句式也会兼容，例如"现在去给鲍鲍考试加油"、"去问问鲍鲍吃药没"。指代不清的句子不会猜，例如"主动去问她"仍会交给普通 AI 对话。
